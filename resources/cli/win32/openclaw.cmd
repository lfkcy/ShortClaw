...
if /i "%1"=="update" (
    echo openclaw is managed by ShortClaw ^(bundled version^).
    echo.
    echo To update openclaw, update ShortClaw:
    echo   Open ShortClaw ^> Settings ^> Check for Updates
    echo   Or download the latest version from https://shortapi.ai/claw
    exit /b 0
)
...
set OPENCLAW_EMBEDDED_IN=ShortClaw
...
if "%_USE_BUNDLED_NODE%"=="1" (
    ...
    "%~dp0..\..\ShortClaw.exe" "%OPENCLAW_ENTRY%" %*
)
