...
# Post-installation script for ShortClaw on Linux
...
# Create symbolic link for ShortClaw app binary
if [ -x /opt/ShortClaw/shortclaw ]; then
    ln -sf /opt/ShortClaw/shortclaw /usr/local/bin/shortclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/ShortClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/ShortClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/ShortClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/ShortClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/shortclaw'
...
echo "ShortClaw has been installed successfully."
