; ShortClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting ShortClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers ShortClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if ShortClaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old ShortClaw was installed
  ...
  ; "Can't modify ShortClaw's files" retry loop -> "ShortClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "ShortClaw 无法关闭" dialog.
...
; "ShortClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with ShortClaw's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove ShortClaw application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\shortclaw (local app data)$\r$\n  • AppData\Roaming\shortclaw (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering ShortClaw processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\shortclaw"
    RMDir /r "$APPDATA\shortclaw"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\shortclaw
    IfFileExists "$LOCALAPPDATA\shortclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\shortclaw"
      IfFileExists "$LOCALAPPDATA\shortclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\shortclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\shortclaw
    IfFileExists "$APPDATA\shortclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\shortclaw"
      IfFileExists "$APPDATA\shortclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\shortclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\shortclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\shortclaw"
    IfFileExists "$APPDATA\shortclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\shortclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\shortclaw"
    RMDir /r "$R3\AppData\Roaming\shortclaw"
