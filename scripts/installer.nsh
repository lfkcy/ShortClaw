...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to completely remove all ShortClaw user data?$\r$\n$\r$\nThis will delete:$\r$\n  • .openclaw folder (configuration & skills)$\r$\n  • AppData\Local\shortclaw (local app data)$\r$\n  • AppData\Roaming\shortclaw (roaming app data)$\r$\n$\r$\nSelect 'No' to keep your data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering ShortClaw processes to release file locks on electron-store
    ; JSON files (settings.json, shortclaw-providers.json, window-state.json, etc.)
    ${nsProcess::FindProcess} "${APP_EXECUTABLE_FILENAME}" $R0
    ${if} $R0 == 0
      ${nsProcess::KillProcess} "${APP_EXECUTABLE_FILENAME}" $R0
    ${endIf}
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R2\.openclaw"
    RMDir /r "$R2\AppData\Local\shortclaw"
    RMDir /r "$R2\AppData\Roaming\shortclaw"
