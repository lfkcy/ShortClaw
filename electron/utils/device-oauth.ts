 ...
 * the Electron IPC system to display UI in the ShortClaw frontend.
 */
...
class DeviceOAuthManager extends EventEmitter {
    ...
    const token: MiniMaxOAuthToken = await this.runWithProxyAwareFetch(() =>
      loginMiniMaxPortalOAuth({
        ...
        openUrl: async (url: string) => {
          ...
          shell
            .openExternal(url)
            .catch((err: unknown) => logger.warn(`[DeviceOAuth] Failed to open browser:`, err));
        },
  ...
  private async onSuccess(
    ...
    // 3. Save provider record in ShortClaw's own store so UI shows it as configured
